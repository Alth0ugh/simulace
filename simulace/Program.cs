...
namespace simulace
{
    ...
    public class Model
    {
        ...
        public Model(Random rnd, int customerCount)
        {
            _random = rnd;
        ...
        public void VytvorProcesy()
        {
          ...
          System.IO.StreamReader("obchod_data.txt");
            ...
            {
                var customerString = string.Format("{0} {1} {2}", Guid.NewGuid(), _random.Next(0, 601), _random.Next(1, 181));
                var shoppingCount = _random.Next(1, 21);
                var shoppingList = new List<string>();
                for (int j = 0; j < shoppingCount; j++)
                {
                    shoppingList.Add(VsechnaOddeleni[_random.Next(0, VsechnaOddeleni.Count)].ID);
                }
                customerString += ' ' + string.Join(' ', shoppingList.ToArray());
                new Zakaznik(this, customerString);
            }
            soubor.Close();
        }
        public int Vypocet()
    ...
    class Program
    {
        static void Main(string[] args)
        {
            ...
            for (int i = 1; i <= 501; i += 10)
            {
                for (int j = 0; j < 10; j++)
                {
                    Model model = new Model(rnd, i);
                    var result = model.Vypocet();
                    Console.Write("{0} KONEC PRO POCET ZAKAZNIKU {1} --------------------------------", result, i);
            ...
            for (int i = 1; i <= 501; i += 10)
            {
                foreach (var entry in results[i])
                {
                    Console.WriteLine("Pre {0} zakaznikov je cas {1}", i, entry);
                }
            }
            Console.ReadLine();
        }
